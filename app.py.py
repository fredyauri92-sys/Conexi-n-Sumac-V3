...
def reproducir_sonido(tipo):
    ...
    """
    st.components.v1.html(js_sonido, height=1, width=1)
...
# Limpieza inteligente de caché y URLs antiguas/guardadas en el navegador
if "api_url" in st.query_params:
    val = st.query_params["api_url"]
    if "AKfycbw-fWPI7iVQTzZRUD" in val or "AKfycbyEtpDsa8tPJ3LKn" in val or not val.strip():
        st.query_params["api_url"] = API_URL_DEFAULT
        st.session_state["api_url"] = API_URL_DEFAULT
    else:
        st.session_state["api_url"] = val
else:
    if "api_url" not in st.session_state:
        st.session_state["api_url"] = API_URL_DEFAULT

if "api_url" in st.session_state:
    val = st.session_state["api_url"]
    if "AKfycbw-fWPI7iVQTzZRUD" in val or "AKfycbyEtpDsa8tPJ3LKn" in val or not val.strip():
        st.session_state["api_url"] = API_URL_DEFAULT

# Panel de Configuración en el Sidebar para Helios y Mozo Administrador
def actualizar_url_conexion(nueva_url):
    url_limpia = nueva_url.strip()
    st.session_state["api_url"] = url_limpia
    st.query_params["api_url"] = url_limpia
...
with st.sidebar:
    ...
    url_input = st.text_input(
        ...
        key="input_url_web_app"
    )
    
    url_actual_cargada = st.session_state.get("url_cargada_actualmente", "")
    if url_input and url_input.strip() != url_actual_cargada:
        actualizar_url_conexion(url_input)
        st.rerun()
        
    if st.button("🔌 Restablecer Enlace por Defecto"):
        actualizar_url_conexion(API_URL_DEFAULT)
        st.rerun()

# Inicializar estado de conexión
if "conexion_fallida" not in st.session_state:
    st.session_state["conexion_fallida"] = False
if "conexion_error_tipo" not in st.session_state:
    st.session_state["conexion_error_tipo"] = None
...
if st.session_state.get("conexion_fallida", False):
    ...
    if error_tipo == "google_login":
        st.markdown("""
            <div class='status-badge' style='background-color: #721C24; color: #F8D7DA; border: 1px solid #F5C6CB; padding: 12px; font-size: 11px; text-align: left; line-height: 1.4; border-radius: 10px; margin-bottom: 15px;'>
                <strong>⚠️ ERROR DE CONFIGURACIÓN EN GOOGLE SHEETS:</strong><br>
                Tu enlace de Google Sheets está solicitando iniciar sesión con una cuenta de Google. Esto ocurre porque:<br>
                1. Al implementar en Apps Script, no seleccionaste <strong>"Cualquier persona" (Anyone)</strong> en "Quién tiene acceso".<br>
                2. Estás usando un correo institucional/corporativo que bloquea accesos públicos. Te sugiero usar una cuenta de correo personal que termine en <strong>@gmail.com</strong>.<br><br>
                <em>Para solucionarlo: En tu laptop, ve a Apps Script ➡️ Implementar ➡️ Administrar implementaciones ➡️ Editar (Lápiz) ➡️ Selecciona 'Nueva versión' y cambia 'Quién tiene acceso' a 'Cualquier persona'. Luego, copia el nuevo enlace y pégalo en el menú lateral.</em>
            </div>
        """, unsafe_allow_html=True)
    else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if isinstance(rows, dict) and "error" in rows:
                st.session_state["conexion_fallida"] = True
                st.session_state["conexion_error_tipo"] = "apps_script_error"
                st.session_state["conexion_error_msg"] = str(rows["error"])
                return None
...
if "datos_cache" not in st.session_state:
    with st.spinner("🔌 Conectando con la Caja Consolidada..."):
        ...
        if datos_nuevos is not None:
            ...
            # conexion_fallida is already set inside cargar_datos_cloud()
    st.session_state["url_cargada_actualmente"] = st.session_state.get("api_url", "")
